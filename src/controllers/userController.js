...
const forgetpassword = async function (req, res) {
  app.post('/forgot-password', (req, res) => {
    ...
    // Store the reset token in the in-memory database (in a real-world scenario, use a database)
    resetTokens.push({ userId: user.id, token: resetToken });
  
    // Simulate sending an email with the reset token (replace this with a real email service)
    console.log(`Reset token for ${email}: ${resetToken}`);
  
    res.json({ message: 'Reset token sent to the registered email address' });
  });
  
  // Forgot password - Step 2: Validate the reset token and update the password
  app.post('/reset-password', (req, res) => {
    const { email, token, newPassword } = req.body;
  
    // Check if the email and token match a user and token in the database
    const tokenRecord = resetTokens.find(t => t.token === token);
    const user = users.find(u => u.id === tokenRecord?.userId && u.email === email);
  
    if (!tokenRecord || !user) {
      return res.status(401).json({ message: 'Invalid reset token or email' });
    }
  
    // Update the user's password (in a real-world scenario, use a secure password hashing library)
    user.password = newPassword;
  
    // Remove the used reset token (in a real-world scenario, handle token expiration and use a database)
    resetTokens.splice(resetTokens.indexOf(tokenRecord), 1);
